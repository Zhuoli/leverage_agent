...
show_help() {
    ...
    echo -e "  ${GREEN}cli${NC}             Launch the terminal chat assistant (Claude by default)"
    ...
    echo -e "  ./chatbot.sh cli                        # Launch terminal chat (Claude)"
    echo -e "  ./chatbot.sh cli --provider openai      # Use OpenAI instead"
    echo -e "  ./chatbot.sh cli --mcp atlassian        # Enable Atlassian MCP"
    echo -e "  ./chatbot.sh cli --help                 # Show all CLI options"
    echo ""
...
show_cli_help() {
    ...
    echo ""
    echo -e "${CYAN}AI Provider Options:${NC}"
    echo -e "  ${GREEN}--provider <name>${NC}       AI provider: claude, openai, oci-openai (default: claude)"
    echo -e "  ${GREEN}--model <name>${NC}          Model name (default: claude-sonnet-4-20250514)"
    echo ""
    echo -e "${CYAN}Message Options:${NC}"
    echo -e "  ${GREEN}-m, --message <msg>${NC}     Send a single message and exit"
    echo -e "  ${GREEN}--test-only${NC}             Test mode (disable MCP and Skills)"
    echo ""
    echo -e "${CYAN}MCP Options:${NC}"
    ...
    echo -e "  ${GREEN}claude${NC}                  Anthropic Claude (default)"
    echo -e "  ${GREEN}openai${NC}                  OpenAI GPT models"
    echo -e "  ${GREEN}oci-openai${NC}              Oracle Cloud Generative AI"
    echo ""
    ...
    echo ""
    echo -e "${CYAN}Examples:${NC}"
    echo -e "  ./chatbot.sh cli                              # Chat with Claude (default)"
    echo -e "  ./chatbot.sh cli --provider openai            # Chat with OpenAI"
    echo -e "  ./chatbot.sh cli --model claude-opus-4-20250514  # Use specific model"
    echo -e "  ./chatbot.sh cli --no-mcp                     # Chat without any MCPs"
    echo -e "  ./chatbot.sh cli --mcp atlassian              # Chat with Atlassian MCP"
